...
from datasets import parkinson_108_dataset as parkinson_dataset
from finetune_trainer_108 import Trainer
from models import model_for_parkinson108 as model_parkinson
...
def main():
    ...
    print(params)
    kq_grid_search = {}
    # search_space_threshold = [0.5]
    # search_space_lr = [1e-3, 3e-4, 5e-4, 2e-3]
    # search_space_dropout = [0.1, 0.2, 0.3, 0.5]
    search_space_threshold = list(np.arange(0.1, 1.0, 0.05))
    ...
    for threshold, lr, dropout in search_space:
        ...
        # torch.cuda.set_devic  e(params.cuda)
        acc, kappa, f1 = 0, 0, 0
        pr_auc, roc_auc = 0, 0
        res = []
        for i in range(3):
