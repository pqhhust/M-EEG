...
dest_dir = '/mnt/disk1/aiotlab/namth/EEGFoundationModel/datasets/parkinson_eegpt_official'

time_window = 10

# %%
labeler = pd.read_csv(f"{root_dir}/labels.csv")
label_dict = {}
label_list = {0: [], 1:[]}
...
def pre_reading_eeg(subject_id):
    ...
    for file in files:
        ...
        a = points % (time_window * 256)
        if a != 0:
            eeg = eeg[:, :-a]
        if eeg.size == 0:
            continue

        eeg = eeg.reshape(chs, -1, time_window, 256).transpose(1, 0, 2, 3)

        eeg_dict[file] = eeg
        file_to_label[file] = label_dict[subject_id]
...
def fold_construct(fold_idx):
    ...
    subjects = {
        'train': label_list[0][:fold_idx] + label_list[0][fold_idx+1:] + label_list[1][:fold_idx] + label_list[1][fold_idx+1:],
        'test': [label_list[0][fold_idx]] + [label_list[1][fold_idx]]
    }
